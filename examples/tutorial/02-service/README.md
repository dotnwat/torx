...
This step deploys a server called `kvd`. It is the tutorial's key-value
server, in [`../kvd`](../kvd/), and it runs on a single node. The job writes
a key through its HTTP API and reads it back. The `kvd` service this step
introduces is simple, but it uses most of torx's programming model, so this
is the longest step in the tutorial.
...
names the binary it runs, and whoever prepared the node guarantees it is
there. For now, that is you.

It is worth stating explicitly: torx does not provision hardware or
software, because there is no generic way to do it. What a suite needs on
its nodes, something outside torx puts there. In practice that is a test
harness around the suite, which is very common, and which step 6
introduces as a launcher. In real distributed testing the machines
themselves may come from a tool like Terraform, and the harness builds on
that.
...
built by `New` with a name and the nodes it needs, and that implements four per-node
hooks. Here the nodes it needs are `torx.Homogeneous(1, torx.NodeSpec{})`, one node of any kind.
`ServiceBase` turns those hooks into the lifecycle the framework
drives around a job:

1. Before the job, for each node: `StopNode`, then `CleanNode`, so that the
   service starts from a known state whatever an earlier run left, then
   `StartNode`. Then `WaitNode` for every node.
2. The job's `Run`.
...
every node on a host, so two services never lease the same port. Then it
...
that address is the loopback and nothing seems to depend on the choice. But on
a pool of real machines, a client on another node reaches the server only
because of it, and step 6, where the nodes are containers, depends on it. The data directory is
...
hanging it. A check torx does not provide is the service's to write: `WaitNode`
is ordinary code, and `torx.WaitUntil` polls any predicate handed to it until
it holds or the deadline passes. `torx.WaitForPort` and `torx.WaitForLog` are
built on it, for a server with no HTTP endpoint.
...
with `jc.Register`. It must be pure: construct and register, and start nothing,
...
If the job fails at once like this instead:
...
kvd is not on your `PATH`, and the service's preflight has said so before
anything started. Assuming `$(go env GOPATH)/bin` is on your `PATH`,
`go install ./examples/tutorial/kvd` puts it there. The tutorial
[README](../README.md) has more on this.
...
- `installKVD` builds kvd into a temporary directory and puts it on the
  test's `PATH`, which the workers and their nodes inherit. It is the test's
