...
// benchmarks, and artifacts. The smoke job from step 2 is joined by a
// benchmark that runs once per point of a parameter matrix, drives kvd's load
// generator on the node, records what it measured, and keeps the raw output
// beside its result; and the service registers kvd's data log for collection
// when a job fails. README.md walks through it.
package main
